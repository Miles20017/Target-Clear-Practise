...
namespace TargetClearCS
{
    internal class Program
    {
        ...
        static int NumberOfPrecedingBanks(List<int> Targets)
        {
            int i;
            for(i=0; i < Targets.Count; i++)
            {
                if (Targets[i] != -1) return i+1;
            }
        ...
        static void PlayGame(List<int> Targets, List<int> NumbersAllowed, bool TrainingGame, int MaxTarget, int MaxNumber)
        {
            ...
            {               
                ...
                if (!UsedJoker) Console.WriteLine("Would you like to use The Joker? (Y/N)");
                string Input = Console.ReadLine();

                if(Input=="Y")
                {
                    int Preceding = NumberOfPrecedingBanks(Targets);
                    for (int i = 0; i < 3; i++)
                    {
                        Targets[RGen.Next(Preceding, Targets.Count())] = -1;
                    }
                    UsedJoker=true;
                    DisplayState(Targets, NumbersAllowed, Score);
                }

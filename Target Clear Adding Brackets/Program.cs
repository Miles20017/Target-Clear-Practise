...
namespace TargetClearCS
{
    internal class Program
    {
        ...
        //Creates two new lists, one for operators (called Operators) and one for the operands (called UserInputInRPN)
        //The subroutine fetches the first operand using GetNumberFromUserInput and always adds this to the list, UserInputInRPN
        //The subroutine then fetches the operator that follows that operand by doing Position - 1, sice GetNumberFromUserInput updates Pos to the next occourance of a number
        //This operator is stored in the Operators list
        //----
        //The subrtouine then fetches the next operand and adds it to UserInputInRPN
        //The subrtouine then fetches the next operator and temporarly stors it in a varliable
        //The subrtouine compares the precedence of the current operator and the one on the list
        //If the current operator has a higher precedence than the last operator added to Operators, It will be added to the top of operators
        //Otherwise all of the operators of lower precidence in the list Operators will be added to UserInputInRPN 
        //---- This loops until the end of the input has been reached
        //Finally add any remaining operators to UserInputInRPN and return the RRN list of strings
        ...
        //Position is updated to 2 indexes after the index of the final digit of the number returned
        //such that when GetNumberFromUserInput("1+1",0) the number returned would be 1 (Index 0) and the value of Pos would be 2 (Index of the second operand)
        //UNLESS the number retrieved was the final number from the expression, then pos would be equal to the length of the input

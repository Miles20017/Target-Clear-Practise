...
namespace TargetClearCS
{
    internal class Program
    {
        ...
        static void PlayGame(List<int> Targets, List<int> NumbersAllowed, bool TrainingGame, int MaxTarget, int MaxNumber)
        {
            ...
            int UndoAvailable = 3;
            Stack<UndoState> Moves = new Stack<UndoState>();
            Moves.Push(new UndoState(NumbersAllowed, Targets, Score));

            while (!GameOver)
            {
                DisplayState(Targets, NumbersAllowed, Score);

                if (Moves.Count() > 1 && UndoAvailable > 0)
                {
                    Console.WriteLine("Would you like to undo the last move? (Y/N)");
                    Console.WriteLine($"You have {UndoAvailable} undo's avilable!");
                ...
                UndoState CurrentState = new UndoState(NumbersAllowed, Targets, Score);

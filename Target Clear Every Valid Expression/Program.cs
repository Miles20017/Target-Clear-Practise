...
namespace TargetClearCS
{
    internal class Program
    {
        ...
        static bool CheckExpression(string UserInput, List<int> Targets)
        {
            if (CheckIfUserInputValid(UserInput))
            {
                int ScoreIDontCareAbout = 0;
                List<string> UserInputInRPN;
                UserInputInRPN = ConvertToRPN(UserInput);
                if (CheckIfUserInputEvaluationIsATarget(Targets, UserInputInRPN, ref ScoreIDontCareAbout))
                {
                    return true;
                }
            }
        ...
        static List<string> EveryOption(List<int> NumbersAllowed, List<int> Targets)
        {

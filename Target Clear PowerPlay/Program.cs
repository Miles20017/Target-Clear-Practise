...
namespace TargetClearCS
{
    internal class Program
    {
        ...
        //-------------Updates-------------
        static bool PowerPlayEnabled(int PowerPLay)
        {
            return (PowerPLay > 0 && PowerPLay < 5) ? true : false;
        }

        static bool PowerPlayAvailable(int PowerPlay)
        {
            return PowerPlay > 0  && !PowerPlayEnabled(PowerPlay)? true :false;
        }
        //--------^----Updates----^--------
        ...
        static void PlayGame(List<int> Targets, List<int> NumbersAllowed, bool TrainingGame, int MaxTarget, int MaxNumber)
        {
            ...
            while (!GameOver)
            {
                ...
                //-------------Updates-------------
                if (PowerPlayAvailable(PowerPlay)) { Console.WriteLine("Enter 'P' to enable PowerPlay! This will disable all point deduction and give you 5 points upon a valid expression!"); }
                else if (PowerPlayEnabled(PowerPlay)) Console.WriteLine("PowerPlayIsEnabled");
                //--------^----Updates----^--------
                ...
                if (CheckIfUserInputValid(UserInput))
                {
                    ...
                    {
                        if (CheckIfUserInputEvaluationIsATarget(Targets, UserInputInRPN, ref Score, PowerPlay))
                        {
                ...
                //-------------Updates-------------
                else if (UserInput == "P" && PowerPlayAvailable(PowerPlay))
                {
                    PowerPlay--; //Enable PowerPlay
                    Console.WriteLine("PowerPlay Enabled...");
                }
                if (PowerPlayEnabled(PowerPlay))
                {
                    Score++;
                    PowerPlay--;
                }
                ...
                {
                    UpdateTargets(Targets, TrainingGame, MaxTarget, PowerPlay);
                }
            }
        ...
        static bool CheckIfUserInputEvaluationIsATarget(List<int> Targets, List<string> UserInputInRPN, ref int Score, int PowerPlay)
        {
        ...
        static void UpdateTargets(List<int> Targets, bool TrainingGame, int MaxTarget, int PowerPlay)
        {
            //-------------Updates-------------
            if (!PowerPlayEnabled(PowerPlay))
            {
